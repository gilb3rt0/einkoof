<script lang="ts">
	import { Checkbox, EinkoofBanner, ErrorMessage, Label, TextInput } from '$lib';
...
<section class="flex flex-col gap-4">
	...
	<form method="POST" action="?/delete">
		<button type="submit" class="rounded-lg bg-red-400 p-2 text-slate-800">delete</button>
	</form>
	<form method="POST" action="?/update" use:enhance class="flex flex-col gap-2">
		...
		{#each $form.items as item, i}
			<div class="group flex flex-col gap-2">
				<Label for={`title-${i}`} error={!!($errors?.items && $errors?.items[i])}
					>item {i + 1}</Label
				>
